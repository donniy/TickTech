...
def parse_body(string, courseid):
    '''
    Parses a given body in string format. Returns the student id, name and label, if possible.
    '''
...
def check_mail(host, port, user, password, course_id):
    ...
    for i in range(mailcount):
        ...
        if (subject == None or body == None or sender == None):
            ...
            newticket = {
                'name': sender, # parsed, but should be compared with database 
                'studentid': studentid, # parsed, but should be compared with database
                'email': address, 
                'subject': subject,
                'message': body,
                'courseid': courseid,
            ...
            result = requests.post('http://localhost:5000/api/ticket/submit', json=newticket)
